...
const signInAndSignUp = (user, authorize, _callback) => {
  ...
  async.waterfall([
    ...
    async (callback) => {
      ...
      let [ err, oauth ] = await To(Oauth.fetchByOpenIdAndSource(authorize.uid, 'weibo'));
    ...
    async (oauth, callback) => {
      ...
      if (user && oauth && oauth.deleted == false) {
        ...
        */
        ...
        let [ err ] = await To(Oauth.save({ data: weibo }));
        if (err) console.log(err);
        callback('binding_finished');
        ...
        */
          ...
          createUser(user, function(newUser){
            ...
            createOauth(user, newUser, function(oauth){
              if (oauth) {
                ...
                uploadImage(user.avatar, newUser._id, function(){
        ...
        getUserInfo(authorize.access_token, authorize.uid, function(info) {
          ...
          createUser(user, async function(newUser){
