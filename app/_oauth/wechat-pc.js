...
import { User, Oauth } from '../modelsa';
...
exports.signin = function(req, res) {
  ...
  if (req.cookies['csrf'] != state) {
    res.redirect(config.domain+'/oauth/wechat-pc');
    return;
  }
  ...
  async.waterfall([
    ...
    // 查询 openid 是否已经存在
    async function(tokenInfo, userinfo, callback) {

      tokenInfo.backup_openid = tokenInfo.openid;
      tokenInfo.openid = userinfo.unionid;

      let [ err, oauth ] = await To(Oauth.fetchByOpenIdAndSource(tokenInfo.openid, 'github'));
      if (err) console.log(err);
    ...
    async function(tokenInfo, userinfo, oauth, callback) {
        ...
        */
        ...
        createUser(newUser, function(user){
          ...
          createOauth(tokenInfo, user, function(oauth){
            if (oauth) {
              if (user.avatar) {
                uploadImage(user.avatar, user._id, function(){
        ...
        createUser(newUser, async function(user){
